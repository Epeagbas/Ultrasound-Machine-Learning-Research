...
#ataPath = '/home/sepeagb2/sam/code/abstractResults/03152020/dataFinal.csv'

dataPath = '/home/sepeagb2/sam/code/abstractResults/03152020/dataSet20_neg20_combined5.csv'
...
def prepareData():
    ...
    image_path='/home/sepeagb2/sam/code/abstractResults/03152020/combined5_cropped/' 
    ...
    for image_id in x:
        ...
        #X.append(cv2.imread(image_path+data['id'][image_id], cv2.IMREAD_COLOR))
    ...
    for image_id in val_x:
        val_add+=1
        print("Append to list: ",val_add,':',str(data['id'][image_id]))
        #Val_x.append(cv2.imread(image_path+data['id'][image_id], cv2.IMREAD_COLOR))
    ...
    for image_id in test_x:
        val_add+=1
        print("Append to list: ",val_add,':',str(data['id'][image_id]))  
        #Test_x.append(cv2.imread(image_path+data['id'][image_id], cv2.IMREAD_COLOR))
...
def trainModel():
    ...
    print("------------------------------------------------------------------------")   
    print("------------------------------------------------------------------------")
    print(np.multiply(y_train,1./20))
    print("------------------------------------------------------------------------")
    print("------------------------------------------------------------------------")
    print("------------------------------------------------------------------------")
    print(y_train/20) 
    ...
    test_y=np.multiply(test_y,1./20)

    #y_train=np.multiply(y_train,1./5)
    
    y_train=np.multiply(y_train,1./20)
    #dataset changed to 5,0,-5
    #val_y=np.multiply(val_y,1./5)
   
    val_y=np.multiply(val_y,1./20)
    ...
    #chagend epoch to 10 from 100
    cb = TimingCallback()
    history=cnn.fit_generator(train_generator, steps_per_epoch=ntrain//batch_size, epochs=25, validation_data=val_generator,
                            validation_steps=nval//batch_size, use_multiprocessing=False, callbacks =[cb] )
    #new version
    ...
    import pandas as pd
    testAnalysis = pd.DataFrame()
    testAnalysis['predict']=model_predict
    testAnalysis['ground_truth']=test_y
    ...
    cnn.save_weights('/home/sepeagb2/sam/code/abstractResults/03152020/weights/model_weights_cropped.h5')
    cnn.save('/home/sepeagb2/sam/code/abstractResults/03152020/model/model_resnet50_cropped.h5')
