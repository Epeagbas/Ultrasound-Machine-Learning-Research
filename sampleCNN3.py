...
def trainModel():
    ...
    print("------------------------------------------------------------------------")
    print(y_train/30) 
    y_train=np.multiply(y_train,1./30)
    val_y=np.multiply(val_y,1./30)
    test_y=np.multiply(test_y,1./30)
    ...
    cnn.compile(loss = 'mean_squared_error',optimizer=optimizers.SGD(lr=1e-3),metrics=['acc'])
